#!/bin/bash
docker rm onlyoffice-deb-builder
docker rm onlyoffice-document-editors-builder
rm -rf build_tools
mv deb_build/document-server-package deb_build/document-server-package.bk
#######################################################################
...
build_oo_binaries() {
  ...
  docker run --name onlyoffice-document-editors-builder -e PRODUCT_VERSION=${_PRODUCT_VERSION} -e BUILD_NUMBER=${_BUILD_NUMBER} -e NODE_ENV='production' -v $(pwd)/${_OUT_FOLDER}:/build_tools/out -v $(pwd)/../server:/server -v $(pwd)/../web-apps:/web-apps onlyoffice-document-editors-builder /bin/bash -c '\
    cd tools/linux && \
...
if [ "${BUILD_DEB}" == "true" ] ; then
  if [ ${build_oo_binaries_exit_value} -eq 0 ] ; then
    ...
    docker run --name onlyoffice-deb-builder \
